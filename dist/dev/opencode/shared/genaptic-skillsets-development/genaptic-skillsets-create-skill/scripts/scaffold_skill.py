...
def semantic_version_key(value: str) -> tuple[int, int, int, tuple[tuple[int, int | str], ...]]:
    """Return a SemVer ordering key without importing project dependencies."""

    match = SEMVER_RE.fullmatch(value)
    if not match:
        raise ScaffoldError(f"invalid Semantic Version in target manifest: {value!r}")
    precedence = value.split("+", 1)[0]
    core, _, prerelease = precedence.partition("-")
...
def parse_manifest(path: Path) -> Pack:
    ...
    for raw in lines:
        top_level = re.match(r"^([a-z][a-z0-9-]*):(?:\s*(.+?))?\s*$", raw)
        if top_level:
            section = top_level.group(1)
            in_latest_release = False
            if section == "source-sha":
                raise ScaffoldError(
                    "target manifest contains retired top-level source-sha; migrate it to "
                    "schema-version 2 publication metadata"
                )
        if raw == "skills:":
            in_skills = True
            in_operations = False
        ...
        if section == "distribution":
            match = re.match(r"^\s{2}visibility:\s*(.+?)\s*$", raw)
            if match:
                visibility = match.group(1).strip().strip("'\"")
        if section == "publication":
            if raw == "  latest-release:":
                in_latest_release = True
                continue
            match = re.match(r"^\s{2}state:\s*(.+?)\s*$", raw)
            if match:
                publication_state = match.group(1).strip().strip("'\"")
            if in_latest_release:
                match = re.match(r"^\s{4}version:\s*(.+?)\s*$", raw)
                if match:
                    latest_release_version = match.group(1).strip().strip("'\"")
        match = re.match(r"^(id|version|maturity):\s*(.+?)\s*$", raw)
        if match:
            values[match.group(1)] = match.group(2).strip().strip("'\"")
    if (
        not values.get("id")
        or not values.get("version")
        or not values.get("maturity")
        or not visibility
        or not publication_state
        or not skills
        or set(operations) != set(OPERATIONS)
    ):
        raise ScaffoldError(
            "cannot parse required schema-v2 lifecycle, skills, and operations from target manifest"
        )
    return Pack(
        values["id"],
        values["version"],
...
def replace_template_tokens(
    ...
    replacements = {
        ...
        'maturity: "draft"': f'maturity: "{pack.maturity}"',
    }
...
def build_plan(root: Path, request_path: Path) -> tuple[dict[str, Any], Pack, Path, bytes, bytes]:
    ...
    if pack.visibility != "public":
        raise ScaffoldError("refusing to add a public skill to a maintainer-only pack")
    if pack.publication_state == "withdrawn":
        raise ScaffoldError("refusing to modify a withdrawn pack")
    if pack.maturity not in {"draft", "release-candidate"}:
        raise ScaffoldError(
            "target pack is frozen; run begin-development with a higher version first"
        )
    if pack.publication_state == "published":
        if pack.latest_release_version is None:
            raise ScaffoldError("published target pack has no latest-release version")
        if semantic_version_key(pack.version) <= semantic_version_key(pack.latest_release_version):
            raise ScaffoldError(
                "published target pack has no newer development version; run "
                "begin-development first"
            )
    ...
    plan = {
        "schemaVersion": 1,
        "operation": "genaptic-skillsets-create-skill",
        "repository": ".",
        "request": "request",
        "skillName": name,
        "packId": pack.id,
        ...
        "notes": [
            ...
            "Add reviewed A-only, B-only, both, and neither cases to the root routing matrix.",
