...
def validate_pack_interface(value: Any, *, declared_skills: set[str]) -> dict[str, Any]:
    data = expect_object(
        value,
        label="interface",
        required={"shortDescription", "starterPrompts"},
    )
    short_description = text_field(
        data["shortDescription"],
        label="interface.shortDescription",
        minimum=25,
        maximum=120,
    )
    if not short_description.endswith((".", "!", "?")):
        raise ScaffoldError("interface.shortDescription must end with sentence punctuation")
    if any(
        marker in short_description.casefold() for marker in ("replace with", "placeholder", "todo")
    ):
        raise ScaffoldError("interface.shortDescription contains unresolved boilerplate")
    raw_prompts = data["starterPrompts"]
    if not isinstance(raw_prompts, list) or not 2 <= len(raw_prompts) <= 4:
        raise ScaffoldError("interface.starterPrompts must contain 2 to 4 items")
    prompts: list[dict[str, str]] = []
    for index, item in enumerate(raw_prompts):
        label = f"interface.starterPrompts[{index}]"
        prompt_data = expect_object(item, label=label, required={"skill", "prompt"})
        skill = text_field(prompt_data["skill"], label=f"{label}.skill", minimum=1, maximum=64)
        if skill not in declared_skills:
            raise ScaffoldError(f"{label}.skill must name a declared skill")
        prompt = text_field(prompt_data["prompt"], label=f"{label}.prompt", minimum=30, maximum=300)
        if f"${skill}" not in prompt:
            raise ScaffoldError(f"{label}.prompt must explicitly name ${skill}")
        if any(marker in prompt.casefold() for marker in ("replace with", "placeholder", "todo")):
            raise ScaffoldError(f"{label}.prompt contains unresolved boilerplate")
        prompts.append({"skill": skill, "prompt": prompt})
    prompt_texts = [item["prompt"] for item in prompts]
    if len(prompt_texts) != len(set(prompt_texts)):
        raise ScaffoldError("interface.starterPrompts prompt text must be unique")
    return {"shortDescription": short_description, "starterPrompts": prompts}
...
def render_manifest(plan: dict[str, Any], maintainer: str) -> str:
    ...
    release_gates = ["native-evidence", "deterministic-archive"]
    if plan["language"] == "rust":
        release_gates.append("rust-assets")
    lines = [
        "schema-version: 2",
        ...
        "  state: unpublished",
        "release-gates:",
        *[f"- {gate}" for gate in release_gates],
        "interface:",
        f"  short-description: {yaml_string(interface['shortDescription'])}",
        "  starter-prompts:",
        *[
            line
            for item in interface["starterPrompts"]
            for line in (
                f"  - skill: {yaml_string(item['skill'])}",
                f"    prompt: {yaml_string(item['prompt'])}",
            )
        ],
        "license: Apache-2.0",
        "maintainers:",
...
This pack owns the approved scope documented in its skillpack design record. Replace this
...
Current source version: `{plan["version"]}`.

Maturity: `release-candidate`. Distribution visibility: `public`. Publication state:
`unpublished`.

Current derived tag: `{plan["packId"]}-v{plan["version"]}`.
...
def replace_template_tokens(
    ...
    *,
    ...
    replacements = {
        ...
        'maturity: "draft"': 'maturity: "release-candidate"',
    }
...
def build_plan(root: Path, request_path: Path) -> dict[str, Any]:
    ...
    original_skillpack_name = text_field(
        request["skillpackName"], label="skillpackName", minimum=3, maximum=100
    )
    ...
    interface = validate_pack_interface(request["interface"], declared_skills=set(names))
    ...
    return {
        "schemaVersion": 1,
        "operation": "genaptic-skillsets-create-skillpack",
        "repository": ".",
        "request": "request",
        "skillpackName": original_skillpack_name,
        "packId": pack_id,
        ...
        "notes": [
            ...
            "Declare reviewed A-only, B-only, both, and neither cases in the root routing matrix.",
...
def apply_plan(root: Path, request_path: Path, expected_digest: str) -> dict[str, Any]:
    ...
    with repository_lock(root) as scratch:
        ...
        ensure_safe_repo_path(root, destination, label="skillpack destination")
        staging_parent = Path(tempfile.mkdtemp(prefix="scaffold-skillpack-", dir=scratch))
...
def parser() -> argparse.ArgumentParser:
    ...
    argument_parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON request matching assets/skillpack-request.template.json.",
    )
    argument_parser.add_argument(
        "--apply",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments.json:
        ...
        print(f"skillpack: {plan['skillpackName']} -> {plan['packId']}")
