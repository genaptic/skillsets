...
class Auditor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if name in {"warnings.warn", "warn"} and not any(
            keyword.arg == "stacklevel" for keyword in node.keywords
        ):
            self.add(
                node,
                "info",
        ...
        if (
            self.except_depth
            and name.endswith((".error", ".warning", ".critical"))
            and not any(keyword.arg == "exc_info" for keyword in node.keywords)
        ):
            self.add(
                node,
                "info",
