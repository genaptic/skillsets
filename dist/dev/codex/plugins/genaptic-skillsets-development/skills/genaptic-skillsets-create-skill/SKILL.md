---
name: genaptic-skillsets-create-skill
description: >
...
- The requested capability is too broad to have one observable outcome; first recommend a
  bounded decomposition, and use `genaptic-skillsets-create-skillpack` if multiple skills
  are required.
- No suitable existing pack owns the capability. Do not create a pack silently; explain the
  mismatch and route to `genaptic-skillsets-create-skillpack`.
...
9. **Author evals before declaring success.** Add at least one explicit positive, two varied
   implicit positives, one contextual positive, and two realistic negatives. Add focused and
   end-to-end behavior cases with observable assertions and prohibited side effects. Add or
   update the canonical root routing boundary for every affected neighbor, with shared A-only,
   B-only, both, and neither prompts. Include a regression case for every risk discovered
   during research.
...
10. **Register canonical changes.** Add the skill once to the target `skillpack.yaml`, update
    ...
    `tests/compatibility/expected-skills.json`, and the reviewed root routing-boundary graph.
    Update schemas, shared policy, or threat documentation only when the new capability
    genuinely changes those contracts. Never
