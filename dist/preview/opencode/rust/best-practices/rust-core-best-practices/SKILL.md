...
- Copy [the baseline-package template](assets/templates/baseline-package/) only as an adaptable,
  complete minimal package.
- Treat [the workspace-member manifest snippet](assets/templates/member-Cargo.toml) as adaptable;
  reconcile it with the target workspace rather than using it as a standalone project.
