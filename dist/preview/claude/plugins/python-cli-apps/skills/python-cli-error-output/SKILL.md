...
description: >
  ...
  python-domain-exception-policy only for explicit internal taxonomy or translation work; an exception
...
- Internal exception taxonomy or translation boundaries and their CLI process mapping must be
  designed together; also use `python-domain-exception-policy`.
...
- The task is exclusively internal exception taxonomy, cleanup, retries, and logging without a
  CLI process mapping; use `python-domain-exception-policy`.
