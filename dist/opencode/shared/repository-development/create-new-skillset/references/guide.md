# Guide: creating a complete skillset

In this repository, “skillset” maps to an independently installable **skillpack**. Use this
guide for decomposition and package-level decisions; follow the ordered procedure in
...
`create-new-skill` when exactly one new public skill belongs in an existing pack. Use
`create-new-skillset` whenever the requested result is a new independently installable and
versioned pack, including a coherent one-skill pack. For that one-skill package, document
the independent installation and release value and name the nearest existing skill outside
the proposed pack that owns the closest excluded outcome.
...
4. Preferred skill in each overlap.
5. Minimum inputs.
...
Every skill needs its own minimum routing and behavior suite. In addition, create hard
pairwise cases for likely competitors:

```text
Skill A positive that mentions Skill B vocabulary
Skill B positive that mentions Skill A vocabulary
A near-neighbor negative for each direction
An overlap case naming the narrower owner
```
