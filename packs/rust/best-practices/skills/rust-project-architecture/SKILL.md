...
- `assets/templates/thin-lib-bin/` is a complete single-package library-plus-binary template.
- `assets/templates/workspace-root/` is a complete minimal virtual-workspace template; rename its
  example packages and reconcile metadata, MSRV, and commands with the target repository.
