...
- The user supplies a skillset name, its details and scope, and an ordered list of one to
...
- Exactly one new skill belongs in an existing skillpack; use `create-new-skill`.
...
- The listed skills are unrelated and cannot share one package purpose, audience, or
  operational boundary; propose separate skillsets rather than forcing false cohesion.
...
- **Skillset name** — proposed public package identity or human-readable name.
...
1. **Normalize the skillset brief.** Capture the exact package identity, details, ownership
   ...
   [`skillset-request.schema.json`](assets/skillset-request.schema.json),
   start from [`skillset-request.template.json`](assets/skillset-request.template.json),
   and consult the [complete example](assets/skillset-request.example.json) when useful.
...
7. **Validate package cohesion and list changes.** Keep skills together only when they share
   ...
   [`skillset-design-record.template.md`](assets/skillset-design-record.template.md). Do not
...
9. **Preview the structural patch.** Run the offline helper or perform an equivalent manual
   ...
   ```bash
   python3 <skill-directory>/scripts/scaffold_skillset.py \
     --repo . --request /path/to/skillset-request.json --json
   ```
   ...
   ```bash
   python3 <skill-directory>/scripts/scaffold_skillset.py \
     --repo . --request /path/to/skillset-request.json \
...
11. **Review cross-skill routing as a system.** Compare all descriptions together. Ensure a
    user can tell which skill owns each outcome, each overlap eval names the narrower
    preferred skill, no skill exists only as a vague catch-all, and the initial discovery
    descriptions remain concise. Add pairwise hard negatives for likely collisions.
...
Before claiming the skillset is complete, confirm:
...
- Normalized skillset brief, identity mapping, and labeled assumptions.
...
- Per-skill routing and behavior eval coverage plus cross-skill overlap analysis.
...
- [Detailed skillset guide](references/guide.md)
- [Completion checklist](references/checklist.md)
- [Primary sources](references/sources.md)
- [Skillset request schema](assets/skillset-request.schema.json)
- [Skillset request template](assets/skillset-request.template.json)
- [Complete request example](assets/skillset-request.example.json)
- [Skillset design record](assets/skillset-design-record.template.md)
- [Skill boundary matrix](assets/skill-boundary-matrix.template.md)
- [Preview-first scaffold helper](scripts/scaffold_skillset.py)
