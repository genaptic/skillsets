...
`create-new-skill` is the narrower route when an existing pack owns one proposed skill.
`create-new-skillset` owns a new installation and release boundary, a coordinated group of
...
| Skill | Focus |
|---|---|
| [`create-new-skill`](skills/create-new-skill/SKILL.md) | Research, design, author, register, and verify exactly one new skill in an existing skillpack. |
| [`create-new-skillset`](skills/create-new-skillset/SKILL.md) | Research, decompose, author, register, and verify one independently installable skillpack and its declared skills. |
...
claude plugin install repository-development@genaptic-skillsets
```

For local changes, regenerate this repository and reinstall the plugin:

```bash
claude plugin uninstall repository-development@genaptic-skillsets
claude plugin install repository-development@genaptic-skillsets
```
...
codex plugin add repository-development@genaptic-skillsets
```

For local changes, regenerate this repository and reinstall the plugin:

```bash
codex plugin remove repository-development@genaptic-skillsets
codex plugin add repository-development@genaptic-skillsets
```
...
```bash
bash dist/install/repository-development.sh --dry-run
```

```powershell
.\dist\install\repository-development.ps1 -DryRun
```
...
Expected release tag (not yet created): `repository-development-v1.0.0`.

No public installation or native-client/model compatibility is claimed yet. See the root
[`COMPATIBILITY.md`](../../../COMPATIBILITY.md) and this pack's structural compatibility
fixture. Paths in this section are relative to the pack location when installed; root links
