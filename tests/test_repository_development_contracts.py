...
def _repository_data() -> dict:
    return {
        ...
        "release": {"license": "Apache-2.0", "initial-year": 2026},
    }
...
def test_canonical_repository_development_contracts_and_source_uris() -> None:
    errors: list[str] = []
    _validate_skill_asset_schemas(ROOT, load_repository(ROOT), errors)
    assert errors == []

    schema_paths = sorted(
        (ROOT / "packs/shared/repository-development/skills").glob("*/assets/*.schema.json")
    )
